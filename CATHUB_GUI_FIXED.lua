...
-- GUI loader with error protection
local UI_URL = "https://raw.githubusercontent.com/tlredz/Library/refs/heads/main/redz-V5-remake/main.luau"

local function LoadCATUI()
    local ok, result = pcall(function()
        local source = game:HttpGet(UI_URL)
        assert(type(source) == "string" and #source > 0, "UI library download returned empty data")

        local chunk, compileError = loadstring(source)
        assert(chunk, compileError or "Could not compile UI library")

        return chunk()
    end)

    if not ok then
        warn("[CAT HUB] GUI library failed to load: " .. tostring(result))
        return nil
    end
...
local redzlib = LoadCATUI()
if not redzlib then
    warn("[CAT HUB] GUI could not start. The UI library URL may be unavailable.")
    return
end
...
if not okWindow or not Window then
    warn("[CAT HUB] MakeWindow failed: " .. tostring(Window))
    return
end
