...
-- CAT HUB custom background
local CAT_HUB_BACKGROUND_ID = "rbxassetid://117196162787674"
local CAT_HUB_GUI_ROOT
local CAT_HUB_ORIGINAL_TRANSPARENCY = {}
local CAT_HUB_BACKGROUND

local function FindCATWindowRoot()
    local playerGui = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui")
    local titleLabel

    for _, obj in ipairs(playerGui:GetDescendants()) do
        if obj:IsA("TextLabel") and obj.Text == "CAT HUB : Blox Fruit" then
            titleLabel = obj
            break
        end
    end
    ...
    if titleLabel then
        local current = titleLabel.Parent
        while current and not current:IsA("ScreenGui") do
            if current:IsA("Frame") then
                CAT_HUB_GUI_ROOT = current
            end
            current = current.Parent
        end
    end

    return CAT_HUB_GUI_ROOT
end

task.spawn(function()
    for _ = 1, 60 do
        local root = FindCATWindowRoot()
        if root then
            local bg = Instance.new("ImageLabel")
            bg.Name = "CAT_HUB_Background"
            bg.BackgroundTransparency = 1
            bg.BorderSizePixel = 0
            bg.Size = UDim2.new(1, 0, 1, 0)
            bg.Position = UDim2.new(0, 0, 0, 0)
            bg.AnchorPoint = Vector2.new(0, 0)
            bg.Image = CAT_HUB_BACKGROUND_ID
            bg.ScaleType = Enum.ScaleType.Crop
            bg.ZIndex = 0
            bg.Active = false
            bg.Parent = root
            CAT_HUB_BACKGROUND = bg

            for _, obj in ipairs(root:GetDescendants()) do
                if obj:IsA("GuiObject") and obj ~= bg then
                    CAT_HUB_ORIGINAL_TRANSPARENCY[obj] = {
...
-- CAT HUB GUI Transparency
Tabs.Misc:AddSlider({
    Name = "GUI Transparency",
    Description = "Adjust CAT HUB window transparency",
    Min = 0,
    Max = 100,
    Increase = 1,
    Default = 0,
    Callback = function(Value)
        ...
        local root = CAT_HUB_GUI_ROOT or FindCATWindowRoot()
        if not root then return end

        for _, obj in ipairs(root:GetDescendants()) do
            if obj:IsA("GuiObject") and obj ~= CAT_HUB_BACKGROUND then
                local original = CAT_HUB_ORIGINAL_TRANSPARENCY[obj]
                if original then
                    obj.BackgroundTransparency = math.clamp(
                        original.BackgroundTransparency + (1 - original.BackgroundTransparency) * transparency,
                        0, 1
                    )
                end
            end
