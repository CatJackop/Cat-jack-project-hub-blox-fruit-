...
Attack.Kill = function(model,Succes)
  ...
  if not Equipped then
    -- Fall back to any usable tool instead of stopping the farm when SelectWeapon is empty/invalid.
...
GetQuests = function()
    ...
    -- Sea 1 custom reworked quest progression supplied by the user.
    if CheckSea(1) and lvl >= 10 then
        local ranges = {
            {10, 29, "JungleQuest"},
            {30, 59, "BuggyQuest1"},
            {60, 89, "DesertQuest"},
            ...
            {450, 474, "SkyQuest"},
            {475, 524, "SkyExp1Quest"},
            {525, 624, "SkyExp2Quest"},
            {625, math.huge, "FountainQuest"},
        }

        local wantedQuest
        for _, info in ipairs(ranges) do
            if lvl >= info[1] and lvl <= info[2] then
                wantedQuest = info[3]
                break
            end
        end

        if wantedQuest and Quests[wantedQuest] then
            -- Exact enemy lists supplied by the user for the corrected Sea 1 stages.
            local allowedMobs = {
                ImpelQuest = { ["Sky Bandit"] = true, ["Dark Master"] = true },
                PrisonQuest1 = { ["Prisoner"] = true, ["Dangerous Prisoner"] = true },
                PrisonerQuest2 = { ["Ruthless Prisoner"] = true, ["Warden"] = true },
                SkyQuest = { ["God's Guard"] = true },
                SkyExp1Quest = { ["Shanda"] = true, ["Wysper"] = true },
                SkyExp2Quest = { ["Royal Squad"] = true, ["Royal Soldier"] = true, ["Thunder God"] = true },
            }
            local allowed = allowedMobs[wantedQuest]

            for id, questInfo in pairs(Quests[wantedQuest]) do
                for mobName, amount in pairs(questInfo.Task or {}) do
                    if amount > 1 and lvl >= questInfo.LevelReq and questInfo.LevelReq >= LevelReq
                        and (not allowed or allowed[mobName]) then
                        LevelReq = questInfo.LevelReq
                        mmb["Mob"] = mobName
                        mmb["NameQuest"] = wantedQuest
                        mmb["ID"] = id
                        mmb["LevelReq"] = questInfo.LevelReq
                    end
                end
            end
        end
    ...
    -- Keep the original quest system for levels below 30 and for Sea 2/Sea 3.
...
GetQuestPoint = function()
    if CheckSea(1) then
        ...
        local customQuestPoints = {
            ...
            SkyQuest = CFrame.new(-4876.26514, 927.40448, -1033.90515, -0.437564254, 5.89331428e-08, 0.899187148, 4.58379112e-08, 1, -4.32347278e-08, -0.899187148, 2.22988881e-08, -0.437564254),
...
-- Improved fruit detector: checks nested map fruit spawns and dropped fruit tools.
local FruitTracker = {Last = nil, Seen = {}}
...
local function IsFruitObject(obj)
    ...
    if string.find(n, "dealer") then return false end
    return string.find(n, "fruit") ~= nil and (obj:IsA("Tool") or obj:IsA("Model") or obj:IsA("BasePart"))
end
...
local function MarkFruit(obj, handle)
    if not _G.FruitESP or not obj or not handle then return end
    if obj:FindFirstChild("CATHUB_FruitHighlight") then return end
    local h = Instance.new("Highlight")
    h.Name = "CATHUB_FruitHighlight"
    h.Adornee = obj
...
local function NotifyFruit(obj, handle)
    if not obj or FruitTracker.Seen[obj] then return end
    local isDropped = obj:IsA("Tool")
    if isDropped and not _G.DroppedFruitNotify then return end
    if not isDropped and not _G.SpawnFruitNotify then return end
    FruitTracker.Seen[obj] = true
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = isDropped and "CAT HUB - Dropped Fruit" or "CAT HUB - Spawn Fruit",
    ...
    while task.wait(1) do
        pcall(function()
            ...
            if fruit and handle then
                ...
                local distance = root and math.floor((root.Position - handle.Position).Magnitude) or 0
                FruitSpawnCheck:SetDesc("Status: ✅ " .. fruit.Name .. " | " .. distance .. " studs")
                MarkFruit(fruit, handle)
                NotifyFruit(fruit, handle)
            else
                FruitSpawnCheck:SetDesc("Status: ❌ No fruit spawned")
            end
        end)
    end
end)
...
-- Auto Farm Magnet Token (inside the Farming section)
local AutoFarmMagnetToken = Tabs.Main:AddToggle({
    Name = "Auto Farm Magnet Token",
    Description = "Automatically visits islands in the current Sea and farms Magnetic-tagged NPCs",
    Default = false,
    Callback = function(Value)
...
task.spawn(function()
    ...
    while task.wait(0.2) do
        if not _G.FarmMagnetToken then continue end

        local currentIslands = IslandData
        if not currentIslands then task.wait(1) continue end

        for islandName, islandData in pairs(currentIslands) do
            if not _G.FarmMagnetToken then break end
            local islandCF = islandData and islandData.CFrame
            if not islandCF then continue end

            pcall(function() _tp(islandCF) end)
            ...
            repeat
                ...
                for _, mob in ipairs(workspace.Enemies:GetChildren()) do
                    ...
                    if mob:IsA("Model") and hum and hrp and hum.Health > 0 then
                        local magnetic = false
                        pcall(function()
                            magnetic = CollectionService:HasTag(mob, "Magnetic") or CollectionService:HasTag(mob, "Magnet")
                        end)
                        if not magnetic then
                            magnetic = mob:GetAttribute("Magnetic") == true
                                or mob:GetAttribute("Magnet") == true
                                or mob:FindFirstChild("Magnetic") ~= nil
                                or mob:FindFirstChild("Magnet") ~= nil
                        end
                        if magnetic then target = mob break end
                    end
                end
            until target or not _G.FarmMagnetToken or tick() >= deadline

            if target and _G.FarmMagnetToken then
                repeat
                    task.wait(0.05)
                    local hum = target:FindFirstChildOfClass("Humanoid")
                    local hrp = target:FindFirstChild("HumanoidRootPart")
                    if not target.Parent or not hum or not hrp or hum.Health <= 0 then break end

                    -- Keep the player in attack range and make sure a weapon is equipped.
                    EquipWeapon(_G.SelectWeapon)
                    local char = plr.Character
                    ...
                    -- Attack.Kill handles positioning; explicitly run the damage routines too.
                    pcall(function() Attack.Kill(target, _G.FarmMagnetToken) end)
                    pcall(function() AttackNoCoolDown() end)
                    pcall(function()
                        if FastAttackModule and FastAttackModule.ExecuteFastAttack then
                            FastAttackModule.ExecuteFastAttack()
                        end
                    end)
                until not _G.FarmMagnetToken or not target.Parent
        ...
        if _G.Level then
            pcall(function()
                ...
                elseif inSub or level < 2600 then
                    ...
                    -- Reworked Sea 1 quests can have a different target name than the
                    -- task name returned by the old quest table. Prefer the exact task
                    -- name, then fall back to the active quest title.
                    local function IsQuestTarget(mob)
                        if not mob or not mob:FindFirstChild("HumanoidRootPart") then return false end
                        if not Attack.Alive(mob) then return false end
                        if mob.Name == enemyName then return true end
                        if QuestTitle ~= "" then
                            local title = string.lower(QuestTitle)
                            local mobName = string.lower(mob.Name)
                            if string.find(title, mobName, 1, true) then
                                return true
                            end
                        end
                        return false
                    ...
                    if target then
                        repeat
                            task.wait(0.15)
                            if not target.Parent or not target:FindFirstChild("HumanoidRootPart") then break end
                            _tp(target.HumanoidRootPart.CFrame * CFrame.new(0,20,0))
                            Attack.Kill(target, _G.Level)
                        until not _G.Level or not questUI.Visible or not target.Parent or not target:FindFirstChild("Humanoid") or target.Humanoid.Health <= 0
                    else
                        -- Wait at the correct enemy spawn until the quest target appears.
...
Description = "Rolls directly without teleporting to the Gacha", 
Default = false,
Callback = function(Value)
  _G.Random_Auto = Value
end})

-- Direct Auto Random Fruit: sends the normal Cousin/Buy request without moving the player.
-- This removes the NPC-distance/teleport requirement from the previous version.
spawn(function()
    local lastBuy = 0
    while task.wait(0.25) do
        if _G.Random_Auto and tick() - lastBuy >= 1 then
            pcall(function()
                replicated.Remotes.CommF_:InvokeServer("Cousin", "Buy")
            end)
            lastBuy = tick()
...
FruitNotifyToggle = Tabs.Raids:AddToggle({
Name = "Fruit Spawn Notification",
Description = "Notify when a map or dropped fruit is detected",
Default = true,
Callback = function(Value)
    _G.FruitNotify = Value
end})

_G.SpawnFruitNotify = true
_G.DroppedFruitNotify = true

Tabs.Raids:AddToggle({
Name = "Spawn Fruit Notifications",
Description = "Notify when a fruit spawns on the map",
Default = true,
Callback = function(Value)
    _G.SpawnFruitNotify = Value
end
})

Tabs.Raids:AddToggle({
Name = "Dropped Fruit Notifications",
Description = "Notify when a dropped fruit is detected",
Default = true,
Callback = function(Value)
    _G.DroppedFruitNotify = Value
end
})

Tabs.Raids:AddButton({
Name = "Test Spawn Fruit Notification",
Description = "Test spawn fruit notification",
Callback = function()
    game:GetService("StarterGui"):SetCore("SendNotification", {Title="CAT HUB - Spawn Fruit", Text="Spawn fruit notifications are working", Duration=NOTIFICATION_DURATION})
end
})

Tabs.Raids:AddButton({
Name = "Test Dropped Fruit Notification",
Description = "Test dropped fruit notification",
Callback = function()
    game:GetService("StarterGui"):SetCore("SendNotification", {Title="CAT HUB - Dropped Fruit", Text="Dropped fruit notifications are working", Duration=NOTIFICATION_DURATION})
end
})

FruitESPToggle = Tabs.Raids:AddToggle({
Name = "Fruit ESP / Highlight",
Description = "Highlight detected fruits",
Default = true,
Callback = function(Value)
    _G.FruitESP = Value
end})
...
Tabs.Raids:AddButton({
    Name = "Tween to Fruit",
    Description = "Tween to the currently detected fruit",
    Callback = function()
        pcall(function()
            local fruit, handle = FindSpawnedFruit()
            if fruit and handle then
...
Callback = function()
    local ok, result = pcall(function()
        ...
        CATNotify("🔄 Manual Hop", "Joining another server...", 5)
