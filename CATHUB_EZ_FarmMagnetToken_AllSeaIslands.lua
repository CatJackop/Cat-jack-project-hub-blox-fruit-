        ...
        if wantedQuest and Quests[wantedQuest] then
            for id, questInfo in pairs(Quests[wantedQuest]) do
                for mobName, amount in pairs(questInfo.Task or {}) do
                    if amount > 1 and questInfo.LevelReq >= LevelReq and lvl >= questInfo.LevelReq then
            ...
            pcall(function()
                ...
                elseif inSub or level < 2600 then
                    ...
                    if target then
                        ...
                        until not _G.Level or not questUI.Visible or not target.Parent or target.Humanoid.Health <= 0
                    else
                        -- If the target is not currently spawned, use the replicated
                        -- enemy location only when an exact target is available.
...
-- Farm Magnet Token: visit every Sea 2 island, look for NPCs tagged "Magnetic",
-- wait briefly for a tagged NPC to spawn, then use the existing attack system to kill it.
local FarmMagnetToken = Tabs.Main:AddToggle({
    Name = "Farm Magnet Token",
    Description = "Tween through every island in the current Sea and farm Magnetic-tagged NPCs",
    Default = false,
    Callback = function(Value)
...
task.spawn(function()
    local CollectionService = game:GetService("CollectionService")
    while task.wait(0.2) do
        if not _G.FarmMagnetToken then
            continue
        end

        -- Use every island in the current Sea, not only Sea 2.
        local CurrentSeaIslands = World1 and Sea1_Islands or (World2 and Sea2_Islands or (World3 and Sea3_Islands))
        if not CurrentSeaIslands then
            task.wait(1)
            continue
        end

        for islandName, islandData in pairs(CurrentSeaIslands) do
            if not _G.FarmMagnetToken then break end

            local islandCF = islandData and islandData.CFrame
            if not islandCF then continue end

            -- Tween to the island first.
            pcall(function()
                _tp(islandCF)
            end)

            -- Give the island a chance to load/spawn its NPCs.
            ...
            repeat
                ...
                for _, mob in ipairs(workspace.Enemies:GetChildren()) do
                    if mob:IsA("Model")
                        ...
                        -- Primary check: Roblox CollectionService tag.
                        pcall(function()
                            magnetic = CollectionService:HasTag(mob, "Magnetic")
                                or CollectionService:HasTag(mob, "Magnet")
                        end)

                        -- Compatibility checks for scripts/games that expose the tag
                        -- as an attribute or marker object instead.
            ...
            until target or not _G.FarmMagnetToken or tick() >= spawnDeadline

            -- Stay on this island while a Magnetic NPC is alive and farm it.
...
Callback = function()
    game:GetService("StarterGui"):SetCore("SendNotification", {Title="CAT HUB - Spawn Fruit", Text="Spawn fruit notifications are working", Duration=5})
end
})
...
Callback = function()
    game:GetService("StarterGui"):SetCore("SendNotification", {Title="CAT HUB - Dropped Fruit", Text="Dropped fruit notifications are working", Duration=5})
end
})
