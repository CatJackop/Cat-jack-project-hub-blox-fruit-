...
GetQuestPoint = function()
    -- Sea 1 AI: never uses saved quest-giver CFrames.
    -- It searches the live NPC models and returns the actual quest-giver position.
    if CheckSea(1) then
        local q = GetQuests()
        if not q or not q.NameQuest then return nil end

        local aliases = {
            BanditQuest1 = {"Bandit Quest Giver 1", "Bandit Quest Giver", "BanditQuest1"},
            JungleQuest = {"Adventurer", "Jungle Quest Giver", "JungleQuest"},
            BuggyQuest1 = {"Buggy Quest Giver", "BuggyQuest1"},
            DesertQuest = {"Desert Adventurer", "Desert Quest Giver", "DesertQuest"},
            SnowQuest = {"Villager", "Snow Quest Giver", "SnowQuest"},
            MarineQuest2 = {"Marine Quest Giver", "MarineQuest2"},
            ImpelQuest = {"Impel Down Quest Giver", "ImpelQuest"},
            PrisonQuest1 = {"Prisoner Quest Giver 1", "Prison Quest Giver", "PrisonQuest1"},
            PrisonerQuest2 = {"Prisoner Quest Giver 2", "Prisoner Quest Giver", "PrisonerQuest2"},
            ColosseumQuest = {"Colosseum Quest Giver", "ColosseumQuest"},
            MagmaQuest = {"Magma Quest Giver", "MagmaQuest"},
            FishmanQuest = {"Fishman Quest Giver", "FishmanQuest"},
            SkyExp1Quest = {"Sky Quest Giver 1", "Sky Expedition Quest Giver 1", "SkyExp1Quest"},
            SkyExp2Quest = {"Sky Quest Giver 2", "Sky Expedition Quest Giver 2", "SkyExp2Quest"},
            FountainQuest = {"Fountain Quest Giver", "FountainQuest"},
        }

        local function norm(v)
            return string.lower((tostring(v):gsub("[%s_%-%d]", "")))
        end

        local wanted = aliases[q.NameQuest] or {q.NameQuest}
        local wantedNorm = {}
        for _, name in ipairs(wanted) do
            wantedNorm[norm(name)] = true
        end

        local containers = {}
        local npcs = workspace:FindFirstChild("NPCs")
        if npcs then table.insert(containers, npcs) end
        local replicatedNPCs = replicated:FindFirstChild("NPCs")
        if replicatedNPCs then table.insert(containers, replicatedNPCs) end
        ...
        for _, container in ipairs(containers) do
            for _, npc in ipairs(container:GetChildren()) do
                ...
                if hrp then
                    local n = norm(npc.Name)
                    local score = 0

                    if wantedNorm[n] then
                        score = score + 100
                    else
                        for aliasNorm in pairs(wantedNorm) do
                            if string.find(n, aliasNorm, 1, true) or string.find(aliasNorm, n, 1, true) then
                                score = score + 60
                            end
                        end
                    end

                    -- Quest NPCs normally expose a ClickDetector or ProximityPrompt.
                    if npc:FindFirstChildWhichIsA("ClickDetector", true) then
                        score = score + 20
                    end
                    if npc:FindFirstChildWhichIsA("ProximityPrompt", true) then
                        score = score + 20
                    end
        ...
        if best and bestScore >= 60 then
            return best.HumanoidRootPart.CFrame
        end

        -- GuideModule fallback: use the live NPC selected by the game's guide system.
        ...
        if _G.Level then
            pcall(function()
                ...
                elseif inSub or level < 2600 then
                    ...
                    -- Sea 1 enemies are acquired dynamically; no saved Sea 1 farm CFrames are used.
