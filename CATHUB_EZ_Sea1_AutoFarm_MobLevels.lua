...
-- CAT HUB: Bypass Quest toggle
_G.BypassQuest = false

local BypassQuestToggle = nil
pcall(function()
    BypassQuestToggle = Tabs.Main:AddToggle({
        Name = "Bypass Quest",
        Description = "Try to start the current quest remotely before walking to the quest giver.",
        Default = false,
        Callback = function(Value)
            _G.BypassQuest = Value
        end
    })
end)

local function TryBypassQuest()
    if not _G.BypassQuest then
        return false
    end

    -- Only use remotes that actually exist in the game.
    -- No remote names are invented/called unless they are present.
    local containers = {
        ...
        if container then
            for _, obj in ipairs(container:GetDescendants()) do
                if obj:IsA("RemoteEvent") then
                    ...
                    if n:find("quest") or n:find("start") then
...
local AutoEquipSelectedItem = Tabs.Main:AddToggle({
    Name = "Auto Equip Selected Item",
    Description = "Automatically equips the currently selected weapon type.",
    Default = true,
    Callback = function(Value)
        _G.AutoEquipSelectedItem = Value
    end
})
_G.AutoEquipSelectedItem = true


-- CAT HUB: Equip only when a valid farm enemy is detected.
local function AutoEquipWhenEnemyDetected(target)
    if not _G.AutoEquipSelectedItem or not target then
        return false
    end
    ...
    local selectedName = _G.SelectWeapon
    if selectedName then
        local equipped = char:FindFirstChild(selectedName)
        if equipped and equipped:IsA("Tool") then
            return true
        end

        local backpackTool = plr.Backpack:FindFirstChild(selectedName)
        if backpackTool and backpackTool:IsA("Tool") then
            playerHum:EquipTool(backpackTool)
            return true
        end
    end

    local selectedType = _G.ChooseWP
    if selectedType then
        local equipped = char:FindFirstChildOfClass("Tool")
        if equipped and equipped.ToolTip == selectedType then
            return true
        end

        for _, tool in ipairs(plr.Backpack:GetChildren()) do
            if tool:IsA("Tool") and tool.ToolTip == selectedType then
                playerHum:EquipTool(tool)
...
local function EquipSelectedItem()
    if not _G.AutoEquipSelectedItem then return false end
    local char = plr.Character
    ...
    if selectedName then
        local tool = plr.Backpack:FindFirstChild(selectedName)
    ...
    if selectedType then
        local equipped = char:FindFirstChildOfClass("Tool")
        if equipped and equipped.ToolTip == selectedType then return true end
        for _, tool in ipairs(plr.Backpack:GetChildren()) do
            if tool:IsA("Tool") and tool.ToolTip == selectedType then
                hum:EquipTool(tool)
                _G.SelectWeapon = tool.Name
...
-- Sea 1 Auto Farm uses live quest-giver and live enemy models only; no preset enemy-position CFrames.
        ...
        if _G.Level then
            pcall(function()
                ...
                elseif inSub or level < 2600 then
                    ...
                    -- AI mode: do not use preset enemy/spawn CFrames.
                    -- If the required NPC is not currently spawned, simply wait and scan again.
                    -- The only combat movement above is based on the live NPC HumanoidRootPart.
                    if not foundMob then
                        task.wait(0.25)
                    end
                end
...
-- ================================================================
-- CAT HUB - NEW SEA 1 REWORK AUTO FARM
-- Quest givers are located by NPC NAME, never by hard-coded NPC CFrame.
-- Enemy positions are also read dynamically from the spawned model.
-- ================================================================
local CAT_Sea1MobLevels = {
    {1, 9, "Bandit"},
    {10, 14, "Monkey"},
    {15, 29, "Gorilla"},
    {30, 39, "Pirate"},
    ...
    {175, 189, "Dark Master"},
    {190, 209, "Prisoner"},
    {210, 224, "Dangerous Prisoner"},
    {225, 249, "Ruthless Prisoner"},
    ...
    {475, 524, "Shanda"},
    {525, 549, "Royal Squad"},
    {550, 624, "Royal Soldier"},
    {625, 649, "Galley Pirate"},
    {650, math.huge, "Galley Captain"},
}

local function CAT_GetSea1MobForLevel(level)
    for _, row in ipairs(CAT_Sea1MobLevels) do
        if level >= row[1] and level <= row[2] then
            return row[3]
        end
    end
    return nil
...
local CAT_QuestGivers = {
    {0, 10, "Bandit Quest Giver"},
    {0, 10, "Marine Leader"},
    {10, 29, "Adventure"},
    {30, 59, "Pirate Adventurer"},
    {60, 89, "Desert Adventurer"},
    {90, 119, "Villager"},
    {120, 149, "Marine"},
    {150, 189, "Sky Adventurer"},
    {190, 219, "Jail Keeper"},
    {220, 249, "Head Jailer"},
    {250, 299, "Colosseum Quest Giver"},
    {300, 374, "The Mayor"},
    {375, 449, "King Neptune"},
    {450, 524, "Mole"},
    {525, 624, "Sky Quest Giver 2"},
    {625, math.huge, "Freezeburg Quest Giver"}
}

local function CAT_GetLevel()
    local data = plr and plr:FindFirstChild("Data")
    local level = data and data:FindFirstChild("Level")
    return level and level.Value or 0
end

local function CAT_GetQuestGiverName(level)
    for _, q in ipairs(CAT_QuestGivers) do
...
local function CAT_FindNPC(name)
...
local function CAT_GetRoot(model)
...
local function CAT_MoveToPart(part)
    ...
    if distance > 8 then
        -- Dynamic CFrame: taken from the actual NPC/enemy currently in the map.
...
local function CAT_InteractNPC(npc)
...
local function CAT_ReadQuestTarget()
    ...
    local raw = label and label.Text
    if not raw or raw == "" then return nil end

    -- The quest UI normally contains the enemy name. Match it against live models.
    for _, enemy in ipairs(workspace:FindFirstChild("Enemies") and workspace.Enemies:GetChildren() or {}) do
        if enemy:IsA("Model") and string.find(string.lower(raw), string.lower(enemy.Name), 1, true) then
            return enemy.Name
        end
    end

    local cleaned = raw:gsub("Defeat", ""):gsub("%d+", ""):gsub("%s+", " "):match("^%s*(.-)%s*$")
    return cleaned ~= "" and cleaned or nil
end

local function CAT_FindTarget(name)
    if not name then return nil end
    ...
    for _, enemy in ipairs(enemies:GetChildren()) do
        if enemy:IsA("Model") and enemy.Name == name then
            local hum = enemy:FindFirstChildOfClass("Humanoid")
            local root = CAT_GetRoot(enemy)
            if hum and root and hum.Health > 0 then
                local d = myRoot and (myRoot.Position - root.Position).Magnitude or 0
                if d < bestDist then
...
local function CAT_EquipSelected()
    local char = plr.Character
    local hum = char and char:FindFirstChildOfClass("Humanoid")
    if not hum then return end
    local wanted = CAT_Sea1Rework.SelectedType
    local current = char:FindFirstChildOfClass("Tool")
    if current and current.ToolTip == wanted then return end
    for _, tool in ipairs(plr.Backpack:GetChildren()) do
        if tool:IsA("Tool") and tool.ToolTip == wanted then
            hum:EquipTool(tool)
            return
        end
    end
...
local function CAT_Attack(target)
    if not target then return end
    CAT_EquipSelected()
    if type(Attack) == "table" and type(Attack.Kill) == "function" then
        pcall(function() Attack.Kill(target, true) end)
        return
    end
    local char = plr.Character
...
-- Optional GUI controls. If the current GUI library exposes Tabs/Main, use it.
pcall(function()
    if Tabs and Tabs.Main and Tabs.Main.AddToggle then
        Tabs.Main:AddToggle("CAT_Sea1_Rework_AutoFarm", {
            Title = "Sea 1 Rework Auto Farm",
            Default = false,
            Callback = function(v) CAT_Sea1Rework.Enabled = v end
        })
        Tabs.Main:AddDropdown("CAT_Sea1_Rework_Weapon", {
            Title = "Sea 1 Weapon",
            Values = {"Melee", "Sword", "Blox Fruit", "Gun"},
            Default = "Melee",
            Callback = function(v) CAT_Sea1Rework.SelectedType = v end
        })
        Tabs.Main:AddSlider("CAT_Sea1_Rework_Speed", {
            Title = "Sea 1 Farm Speed",
            Default = 120,
            Min = 10,
            Max = 180,
            Rounding = 0,
            Callback = function(v) CAT_Sea1Rework.Speed = v end
        })
    end
end)

-- Main loop: level -> find quest giver by name -> interact -> read target -> find live enemy -> attack.
task.spawn(function()
    while task.wait(0.15) do
        if not CAT_Sea1Rework.Enabled then continue end
        pcall(function()
            local level = CAT_GetLevel()
            local giverName = CAT_GetQuestGiverName(level)
            CAT_Sea1Rework.QuestGiver = giverName or "None"

            local targetName = CAT_GetSea1MobForLevel(CAT_GetLevel()) or CAT_ReadQuestTarget()
            if targetName then
                CAT_Sea1Rework.TargetName = targetName
            end

            local target = CAT_FindTarget(CAT_Sea1Rework.TargetName)
            if target then
                local root = CAT_GetRoot(target)
                if root then
                    CAT_MoveToPart(root)
                    CAT_Attack(target)
                end
            else
                local giver = CAT_FindNPC(giverName)
                local root = CAT_GetRoot(giver)
                if root then
                    if CAT_MoveToPart(root) then
                        CAT_InteractNPC(giver)
                    end
                end
            end
        end)
