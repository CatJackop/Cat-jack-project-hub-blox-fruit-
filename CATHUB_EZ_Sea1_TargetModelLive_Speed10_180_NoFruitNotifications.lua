...
-- Live Target Model display: follows the currently selected Auto Farm Level quest.
local TargetModelStatus = Tabs.Main:AddParagraph("Target Model", "Waiting for quest...")

task.spawn(function()
    local lastTarget = ""
    while task.wait(0.25) do
        local target = "Waiting for quest..."
        pcall(function()
            local q = QuestNeta and QuestNeta()
            if q and q[1] and q[1] ~= "" then
                target = tostring(q[1])
            elseif getgenv().SelectMob and getgenv().SelectMob ~= "" then
                target = tostring(getgenv().SelectMob)
            end
        end)
        if target ~= lastTarget then
            lastTarget = target
            pcall(function()
                TargetModelStatus:SetDesc(target)
            end)
        end
    end
end)
