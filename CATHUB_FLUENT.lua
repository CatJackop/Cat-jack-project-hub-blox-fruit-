...
-- NPC NAME ↔ POSITION DIAGNOSTIC
-- Keeps each detected NPC instance paired with its own current position for UI/debugging.
local NPCPositionMap = setmetatable({}, {__mode = "k"})

local function TrackNPCPosition(npc)
    ...
    NPCPositionMap[npc] = record
    return record
end

local function GetNPCPositionRecord(npc)
    return TrackNPCPosition(npc) or NPCPositionMap[npc]
end

-- Refreshes records when NPCs are added or removed. This is display/debug data only.
task.spawn(function()
...
BringEnemy = function(Mon)
    ...
    -- If no target is supplied, use the closest alive enemy.
    if not Mon then
    ...
    local maxDistance = tonumber(_G.BringDistance) or 300

    -- Bone Farm: bring all supported Haunted Castle bone mobs together.
    local BoneMobs = {
        ["Reborn Skeleton"] = true,
        ["Living Zombie"] = true,
        ["Demonic Soul"] = true,
        ["Possessed Mummy"] = true,
        ["Posessed Mummy"] = true -- compatibility with the older spelling
    }
    local boneMode = _G.AutoFarm_Bone and BoneMobs[Mon.Name]

    for _, enemy in ipairs(workspace.Enemies:GetChildren()) do
        local shouldBring = false
        if enemy ~= Mon then
            if boneMode then
                shouldBring = BoneMobs[enemy.Name] == true
            else
                shouldBring = enemy.Name == Mon.Name
            end
        end
...
GetQuests = function()
    ...
    if lvl >= 625 and CheckSea(1) then
    ...
    -- Force the requested Sea 1 Auto Farm quest ranges on the reworked map.
    if CheckSea(1) then
        local ranges = {
            {30, 59, "BuggyQuest1"},
            {60, 89, "DesertQuest"},
            {90, 119, "SnowQuest"},
            {120, 149, "MarineQuest2"},
            {150, 189, "ImpelQuest"},
            {190, 209, "PrisonQuest1"},
            {210, 249, "PrisonerQuest2"},
            {250, 299, "ColosseumQuest"},
            {300, 374, "MagmaQuest"},
            {375, 449, "FishmanQuest"},
            {450, 524, "SkyExp1Quest"},
            {525, 624, "SkyExp2Quest"},
            {625, math.huge, "FountainQuest"}
        }
        for _, range in ipairs(ranges) do
            if lvl >= range[1] and lvl <= range[2] then
                mmb["NameQuest"] = range[3]
                mmb["LevelReq"] = range[1]
                break
            end
        end
    end
...
-- Custom Sea 1 quest-giver CFrames for the reworked map.
-- These override the old NPC lookup only for the listed Sea 1 quests.
local Sea1QuestCFrames = {
    JungleQuest = CFrame.new(1054.14734, 14.8172684, 1551.94543),
    BuggyQuest1 = CFrame.new(-1152.64917, 17.3088741, 3853.4502),
    DesertQuest = CFrame.new(925.731812, 5.91223907, 4201.32178),
    SnowQuest = CFrame.new(1396.56726, 78.0434265, -1307.08447),
    MarineQuest2 = CFrame.new(-4697.96533, 5.89573383, 4221.63086),
    ImpelQuest = CFrame.new(-4748.42676, 967.410156, -744.701843),
    PrisonQuest1 = CFrame.new(5205.03711, 19.5656109, 734.285645),
    PrisonerQuest2 = CFrame.new(5319.99268, 19.56563, 862.444275),
    ColosseumQuest = CFrame.new(-1344.94727, 12.4950104, -2926.1123),
    MagmaQuest = CFrame.new(-5305.25195, 18.483923, 8477.83398),
    FishmanQuest = CFrame.new(61400.6445, 24.9472847, 1627.81384),
    SkyExp1Quest = CFrame.new(-5954.75391, 5469.16748, 2084.41553),
    SkyExp2Quest = CFrame.new(-7029.29785, 5591.91846, 1348.11267),
    FountainQuest = CFrame.new(5258.60938, 76.1030502, 4090.62671)
}

GetQuestPoint = function()
    if World1 then
        local questData = GetQuests()
        if questData and Sea1QuestCFrames[questData.NameQuest] then
            return Sea1QuestCFrames[questData.NameQuest]
        end
    end
...
-- CAT HUB Fluent UI adapter
local Fluent = loadstring(game:HttpGetAsync("https://github.com/Soluna-Development/Library/releases/latest/download/Fluent.luau", true))()
...
-- Compatibility adapter: keeps the existing CAT HUB feature code unchanged.
local Compat = {}
function Compat:MakeTab(opts)
    local tab = Window:AddTab({Title = opts.Title or "Tab", Icon = opts.Icon})
    local adapter = {}
    function adapter:AddSection(title)
        return tab:AddSection(title)
    end
    function adapter:AddParagraph(title, content)
        local paragraph = tab:AddParagraph({Title = title or "", Content = content or ""})
        return {SetDesc = function(_, value)
            pcall(function() paragraph:SetDesc(value) end)
            pcall(function() paragraph:SetTitle(value) end)
            pcall(function() paragraph:SetContent(value) end)
        end}
    end
    function adapter:AddToggle(opts2)
        return tab:AddToggle(opts2.Title or "Toggle", {Title = opts2.Title or "Toggle", Default = opts2.Default or false}):OnChanged(function(v)
            if opts2.Callback then opts2.Callback(v) end
        end)
    end
    function adapter:AddButton(opts2)
        return tab:AddButton({Title = opts2.Title or "Button", Description = opts2.Description, Callback = opts2.Callback})
    end
    function adapter:AddSlider(opts2)
        return tab:AddSlider(opts2.Title or "Slider", {Title=opts2.Title or "Slider", Default=opts2.Default or opts2.Value or opts2.Min or 0, Min=opts2.Min or 0, Max=opts2.Max or 100, Rounding=opts2.Rounding or 0}):OnChanged(function(v)
            if opts2.Callback then opts2.Callback(v) end
        end)
    end
    function adapter:AddDropdown(opts2)
        local dd = tab:AddDropdown(opts2.Title or "Dropdown", {Title=opts2.Title or "Dropdown", Values=opts2.Values or opts2.Options or {}, Multi=opts2.Multi or false, Default=opts2.Default})
        if opts2.Callback then dd:OnChanged(opts2.Callback) end
        return dd
    end
    function adapter:AddTextBox(opts2)
        return tab:AddInput(opts2.Title or "Input", {Title=opts2.Title or "Input", Default=opts2.Default or "", Placeholder=opts2.Placeholder or "", Numeric=opts2.Numeric or false, Finished=opts2.Finished or false}):OnChanged(function(v)
            if opts2.Callback then opts2.Callback(v) end
        end)
    end
    function adapter:AddDiscordInvite(opts2)
        return tab:AddParagraph({Title = opts2.Title or "CAT HUB", Content = opts2.Description or opts2.Invite or ""})
    end
    return adapter
end
function Compat:Notify(opts)
    return Fluent:Notify(opts)
end
function Compat:SetUIScale(_) end

Window = Compat
local redzlib = Compat
local Minimizer = nil
...
local FruitSpawnCheck = Tabs.Info:AddParagraph("🍈 Fruit Spawn", "Checking...")


-- Magnetic Event Enemy Detector (detection/alerts only; no combat automation)
local MagneticEventCheck = Tabs.Info:AddParagraph("🧲 Magnetic Event", "Status: Waiting for event enemies...")
_G.MagneticEventDetector = (_G.MagneticEventDetector ~= false)

local function IsMagneticEventEnemy(enemy)
    if not enemy or not enemy:IsA("Model") then return false end
    local name = string.lower(enemy.Name)
    -- Matches known/labelled Magnetic Event enemies without relying on a hard-coded NPC name.
    return name:find("magnetic", 1, true) ~= nil
        or name:find("magnet", 1, true) ~= nil
end

local function RefreshMagneticEventStatus()
    local found = {}
    for _, enemy in ipairs(workspace.Enemies:GetChildren()) do
        if IsMagneticEventEnemy(enemy) then
            table.insert(found, enemy.Name)
        end
    end

    if #found > 0 then
        MagneticEventCheck:SetDesc("Status: 🧲 ACTIVE | Enemies: " .. table.concat(found, ", "))
        return found
    else
        MagneticEventCheck:SetDesc("Status: ⏳ Waiting for Magnetic Event enemies...")
        return nil
    end
end
...
workspace.Enemies.ChildAdded:Connect(function(enemy)
    task.wait(0.2)
    if _G.MagneticEventDetector and IsMagneticEventEnemy(enemy) and not MagneticSeen[enemy] then
        MagneticSeen[enemy] = true
        RefreshMagneticEventStatus()
        pcall(function()
            Fluent:Notify({
                Title = "🧲 CAT HUB | Magnetic Event",
                Content = "Magnetic Event enemy detected: " .. enemy.Name,
                Duration = 5
            })
        end)
        enemy.AncestryChanged:Connect(function(_, parent)
            if not parent then
                MagneticSeen[enemy] = nil
                if _G.MagneticEventDetector then RefreshMagneticEventStatus() end
            end
        end)
    end
end)
...
task.spawn(function()
    while task.wait(2) do
        if _G.MagneticEventDetector then
            RefreshMagneticEventStatus()
        else
            MagneticEventCheck:SetDesc("Status: Detector OFF")
        end
    end
end)

Tabs.Info:AddToggle({
    Title = "🧲 Magnetic Event Enemy Detector",
    Default = _G.MagneticEventDetector,
    Callback = function(Value)
        _G.MagneticEventDetector = Value
        if Value then
            RefreshMagneticEventStatus()
            pcall(function() Fluent:Notify({Title = "CAT HUB", Content = "Magnetic Event Detector ON", Duration = 2}) end)
        else
            MagneticEventCheck:SetDesc("Status: Detector OFF")
            pcall(function() Fluent:Notify({Title = "CAT HUB", Content = "Magnetic Event Detector OFF", Duration = 2}) end)
        end
    end
})
...
-- Magnetic Event quick-access button (detect/select/highlight only; no movement or combat)
local function GetNearestMagneticEnemy()
    local character = plr.Character
    local root = character and character:FindFirstChild("HumanoidRootPart")
    local bestEnemy, bestDistance

    for _, enemy in ipairs(workspace.Enemies:GetChildren()) do
        if IsMagneticEventEnemy(enemy) then
            local humanoid = enemy:FindFirstChildOfClass("Humanoid")
            local enemyRoot = enemy:FindFirstChild("HumanoidRootPart") or enemy.PrimaryPart
            if (not humanoid or humanoid.Health > 0) and enemyRoot then
                local distance = root and (root.Position - enemyRoot.Position).Magnitude or math.huge
                if not bestDistance or distance < bestDistance then
                    bestEnemy, bestDistance = enemy, distance
                end
            end
        end
    end
...
local function HighlightMagneticEnemy(enemy)
    if not enemy then return end
    local old = enemy:FindFirstChild("CAT_HUB_MagneticHighlight")
    if old then old:Destroy() end

    local highlight = Instance.new("Highlight")
    highlight.Name = "CAT_HUB_MagneticHighlight"
...
Tabs.Info:AddButton({
    Title = "🧲 Find Magnetic Enemy",
    Description = "Selects and highlights the nearest detected Magnetic Event enemy. Does not move or attack.",
    Callback = function()
        local enemy, distance = GetNearestMagneticEnemy()
        if enemy then
            HighlightMagneticEnemy(enemy)
            local text = "🧲 Magnetic enemy found: " .. enemy.Name
            if distance and distance < math.huge then
                text = text .. " | Distance: " .. math.floor(distance) .. " studs"
            end
            MagneticEventCheck:SetDesc("Status: 🧲 TARGET DETECTED | " .. enemy.Name)
            pcall(function()
                Window:Notify({
            ...
            MagneticEventCheck:SetDesc("Status: ⏳ No Magnetic Event enemy detected")
            pcall(function()
                Window:Notify({
                    Title = "🧲 Magnetic Event",
                    Content = "No Magnetic Event enemies are currently detected.",
                    Image = "rbxassetid://72243650390596",
                    Duration = 4
                })
            end)
...
_G.FruitNotifier = (_G.FruitNotifier == true)
...
local function FindSpawnedFruit()
    -- Only count an actual dropped fruit, not map objects, NPCs, dealers,
    -- folders, or anything that merely has "Fruit" in its name.
    for _, obj in ipairs(workspace:GetChildren()) do
        if obj:IsA("Tool") then
            local handle = obj:FindFirstChild("Handle")
            local eatRemote = obj:FindFirstChild("EatRemote", true)

            if handle and eatRemote and not obj:IsDescendantOf(plr.Character) then
                return obj.Name
            end
        end
    end
    ...
    while task.wait(1) do
        pcall(function()
            ...
            if _G.FruitNotifier then
                local fruit = FindSpawnedFruit()
                if fruit then
                    FruitSpawnCheck:SetDesc("Status: 🍈 " .. fruit .. " was dropped/spawned in the map!")
                else
                    FruitSpawnCheck:SetDesc("Status: ❌ No fruit in the map")
                end
            else
                FruitSpawnCheck:SetDesc("Status: Fruit notifier OFF")
            end
        end)
    end
end)
...
Tabs.Main:AddToggle({
    Name = "Auto Farm Bone",
    Description = "",
    Default = CAT_HUB_Settings and CAT_HUB_Settings.AutoFarmBone or false,
    Callback = function(Value)
        _G.AutoFarm_Bone = Value
        if CAT_HUB_Settings then CAT_HUB_Settings.AutoFarmBone = Value end
    end
})
...
spawn(function()
    ...
    local BonesTable = {
        ...
        "Demonic Soul",
        "Possessed Mummy",
        "Posessed Mummy" -- compatibility with older spelling
    }
    ...
    while wait(0.5) do
        ...
        pcall(function()
            ...
            for _, enemy in ipairs(workspace.Enemies:GetChildren()) do
                if table.find(BonesTable, enemy.Name) then
                    local hum = enemy:FindFirstChildOfClass("Humanoid")
            ...
            if bone then
                ...
                local patrolPoints = {
                    CFrame.new(-9495.6806640625, 453.58624267578125, 5977.3486328125),
                    CFrame.new(-9495.6806640625, 172, 6078),
                    CFrame.new(-9495.6806640625, 85, 6078),
                    CFrame.new(-9579, 6, 6194)
                }
...
-- CAT HUB manual save system (specific settings only)
local CAT_HUB_SAVE_FOLDER = "CAT HUB"
local CAT_HUB_SAVE_SUBFOLDER = CAT_HUB_SAVE_FOLDER .. "/Settings"
local CAT_HUB_SAVE_FILE = CAT_HUB_SAVE_SUBFOLDER .. "/SpecificSettings.json"

local function CAT_HUB_EnsureSaveFolder()
    if makefolder then
        if isfolder and not isfolder(CAT_HUB_SAVE_FOLDER) then pcall(makefolder, CAT_HUB_SAVE_FOLDER) end
        if isfolder and not isfolder(CAT_HUB_SAVE_SUBFOLDER) then pcall(makefolder, CAT_HUB_SAVE_SUBFOLDER) end
    end
end
CAT_HUB_EnsureSaveFolder()
local CAT_HUB_Settings = {
    FruitNotifier = false,
    AutoFarmBone = false,
    AcceptBoneQuest = false
}

local function CAT_HUB_LoadSettings()
    if readfile and isfile and isfile(CAT_HUB_SAVE_FILE) then
        local ok, data = pcall(function()
            return game:GetService("HttpService"):JSONDecode(readfile(CAT_HUB_SAVE_FILE))
        end)
        if ok and type(data) == "table" then
            for k, v in pairs(data) do
                CAT_HUB_Settings[k] = v
            end
        end
    end
end

local function CAT_HUB_SaveSettings()
    if writefile then
        local ok = pcall(function()
            writefile(CAT_HUB_SAVE_FILE, game:GetService("HttpService"):JSONEncode(CAT_HUB_Settings))
        end)
        return ok
    end
    return false
...
CAT_HUB_LoadSettings()

Tabs.Settings:AddSection("CAT HUB Specific Save")
Tabs.Settings:AddToggle({
    Name = "Fruit Notifier",
    Description = "Notify when an actual fruit is dropped/spawned in the map",
    Default = CAT_HUB_Settings.FruitNotifier == true,
    Callback = function(Value)
        _G.FruitNotifier = Value
        CAT_HUB_Settings.FruitNotifier = Value
        if Fluent and Fluent.Notify then
            Fluent:Notify({Title = "CAT HUB", Content = Value and "Fruit Notifier ON" or "Fruit Notifier OFF", Duration = 2})
        end
    end
})
...
Tabs.Settings:AddButton({
    ...
    Description = "Saves Fruit Notifier and Bone Farm settings",
    Callback = function()
        CAT_HUB_Settings.FruitNotifier = (_G.FruitNotifier == true)
        CAT_HUB_Settings.AutoFarmBone = (_G.AutoFarm_Bone == true)
        CAT_HUB_Settings.AcceptBoneQuest = (_G.AcceptQuestB == true)
        local saved = CAT_HUB_SaveSettings()
        if Fluent and Fluent.Notify then
            Fluent:Notify({Title = "CAT HUB", Content = saved and "Settings saved!" or "Executor does not support saving.", Duration = 3})
        end
    end
})
Tabs.Settings:AddButton({
    Name = "Load Selected Settings",
    Description = "Loads your previously saved settings",
    Callback = function()
        CAT_HUB_LoadSettings()
        _G.FruitNotifier = CAT_HUB_Settings.FruitNotifier == true
        _G.AutoFarm_Bone = CAT_HUB_Settings.AutoFarmBone == true
        _G.AcceptQuestB = CAT_HUB_Settings.AcceptBoneQuest == true
        if Fluent and Fluent.Notify then
            Fluent:Notify({Title = "CAT HUB", Content = "Saved settings loaded!", Duration = 3})
        end
    end
})
...
Tabs.Settings:AddSlider({
    ...
    Description = "Maximum distance used when bringing matching mobs",
    Min = 50,
    Max = 300,
    Default = 300,
    Increment = 10,
...
-- =========================================================
-- CAT HUB : Smart Server Hop + JobId Notifications
-- =========================================================
...
local function CATNotify(title, content, duration)
    pcall(function()
        Window:Notify({
            Title = title or "CAT HUB",
            Content = content or "",
            Image = "rbxassetid://72243650390596",
...
local function CATCopyJobId()
    local jobId = tostring(game.JobId or "")
    if jobId == "" then
        CATNotify("❌ JobId", "JobId is unavailable.", 4)
        return
    end
    ...
    if copied then
        CATNotify("📋 JobId Copied", "Suitable server JobId copied:\n" .. jobId, 6)
    else
        CATNotify("📋 JobId", "Suitable server:\n" .. jobId, 6)
    end
end

local function CATFruitFound()
    for _, obj in ipairs(workspace:GetChildren()) do
...
local function CATMirageFound()
    ...
    return (workspace:FindFirstChild("Map") and workspace.Map:FindFirstChild("MysticIsland"))
        or (locations and locations:FindFirstChild("Mirage Island", true))
end

local function CATPrehistoricFound()
    local origin = workspace:FindFirstChild("_WorldOrigin")
    local locations = origin and origin:FindFirstChild("Locations")
    return (workspace:FindFirstChild("Map") and workspace.Map:FindFirstChild("PrehistoricIsland"))
        or (locations and locations:FindFirstChild("Prehistoric Island", true))
end

local function CATNearFullMoon()
    local moon1 = "http://www.roblox.com/asset/?id=9709135895"
    local moon2 = "http://www.roblox.com/asset/?id=9709139597"
    local moon3 = "http://www.roblox.com/asset/?id=9709143733"
    local moon4 = "http://www.roblox.com/asset/?id=9709149052"
    ...
    -- Near full moon = 3/8, 4/8 (full), or 5/8 (last night).
    return moon == moon3 or moon == moon4 or moon == moon5
end

local function CATSuitable()
    if CAT_ServerHop.Mode == "Moon" then
        return CATNearFullMoon(), "Near Full Moon"
    elseif CAT_ServerHop.Mode == "Mirage" then
        return CATMirageFound() ~= nil, "Mirage Island"
    elseif CAT_ServerHop.Mode == "Prehistoric" then
        return CATPrehistoricFound() ~= nil, "Prehistoric Island"
    elseif CAT_ServerHop.Mode == "Fruit" then
        local fruit = CATFruitFound()
        return fruit ~= nil, fruit
    end

    return false, nil
end

local function CATGetServerList()
    local HttpService = game:GetService("HttpService")
...
local function CATFindNextServer()
    local servers = CATGetServerList()
    local candidates = {}
    ...
    for _, server in ipairs(servers) do
        ...
        if id ~= "" and id ~= game.JobId
            ...
            and not CAT_ServerHop.Visited[id] then
            table.insert(candidates, server)
        end
    end

    if #candidates == 0 then
        CAT_ServerHop.Visited = {}
        for _, server in ipairs(servers) do
            local id = tostring(server.id or "")
            if id ~= "" and id ~= game.JobId
...
local function CATStartHop(mode, label)
    if CAT_ServerHop.Running then
        CATNotify("🔎 Server Hop", "A server search is already running.", 3)
        return
    end

    CAT_ServerHop.Running = true
    CAT_ServerHop.Mode = mode
    CAT_ServerHop.Visited = {}

    CATNotify("🔎 Searching", "Searching for " .. label .. " server...", 4)

    task.spawn(function()
        local attempts = 0

        while CAT_ServerHop.Running and attempts < 25 do
            attempts += 1

            local found, detail = CATSuitable()
            if found then
                CAT_ServerHop.Running = false

                local detailText = tostring(detail or label)
                CATNotify(
                    "✅ Suitable Server Found",
                    label .. " found!\n" ..
                    "Server: " .. detailText .. "\n" ..
            ...
            CATNotify(
                ...
                tostring(attempts) .. "/25)",
                3
            )

            local server = CATFindNextServer()
            if not server then
                CAT_ServerHop.Running = false
                CATNotify("❌ No Server", "No suitable public server was found.", 5)
                return
            end

            CAT_ServerHop.Visited[tostring(server.id)] = true
            ...
            if not ok then
                CATNotify("⚠️ Hop Failed", "Could not join the selected server. Retrying...", 4)
                task.wait(1)
            else
                -- The game normally changes place after this call.
                task.wait(8)
            end
        end

        CAT_ServerHop.Running = false
        CATNotify("❌ Search Ended", "No suitable server found after 25 attempts.", 5)
    end)
end

local function CATStopHop()
    CAT_ServerHop.Running = false
    CAT_ServerHop.Mode = nil
    CATNotify("⏹️ Server Hop", "Server search stopped.", 3)
end

Tabs.Race:AddSection("Smart Server Hop")

Tabs.Race:AddButton({
    Name = "🌕 Near Full Moon Hop",
    Description = "Hop until a near-full-moon server is found, then copy its JobId.",
    Callback = function()
        CATStartHop("Moon", "Near Full Moon")
    end
})

Tabs.Race:AddButton({
    Name = "🌫️ Mirage Island Hop",
    Description = "Hop until Mirage Island is detected, then copy the JobId.",
    Callback = function()
        CATStartHop("Mirage", "Mirage Island")
    end
})

Tabs.Race:AddButton({
    Name = "🦖 Prehistoric Island Hop",
    Description = "Hop until Prehistoric Island is detected, then copy the JobId.",
    Callback = function()
        CATStartHop("Prehistoric", "Prehistoric Island")
    end
})

Tabs.Race:AddButton({
    Name = "🍎 Fruit Spawn Hop",
    Description = "Hop until a fruit is detected in the current server, then copy the JobId.",
    Callback = function()
        CATStartHop("Fruit", "Fruit Spawn")
    end
})

Tabs.Race:AddButton({
    Name = "📋 Copy Current JobId",
    Description = "Copy the current server JobId.",
    Callback = function()
        CATCopyJobId()
    end
})
...
Tabs.Race:AddButton({
    Name = "⏹️ Stop Server Hop",
    Description = "Stop the active smart server search.",
    Callback = function()
        CATStopHop()
    end
})
...
Window:Notify({
  Title = "CAT HUB",
  Content = "CAT hub da comeback",
  Image = "rbxassetid://72243650390596",
  Duration = 5
})

-- CAT HUB | Magnetic Enemy Detector / Notification / Marker (safe utility)
do
    local Players = game:GetService("Players")
    local Workspace = game:GetService("Workspace")
    local StarterGui = game:GetService("StarterGui")
    ...
    local function isMagnetic(model)
        if not model or not model:IsA("Model") then return false end
        local n = string.lower(model.Name)
        return string.find(n, "magnetic", 1, true) ~= nil or string.find(n, "magnet", 1, true) ~= nil
    end
    ...
    local function markEnemy(enemy)
        if markers[enemy] then return end
        local highlight = Instance.new("Highlight")
        highlight.Name = "CAT_HUB_MagneticMarker"
        highlight.Adornee = enemy
        highlight.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
        highlight.Parent = enemy
        markers[enemy] = highlight
    end

    local function unmarkEnemy(enemy)
        local h = markers[enemy]
    ...
    local function scan()
        ...
        for _, obj in ipairs(Workspace:GetDescendants()) do
            if isMagnetic(obj) then
                local hum = obj:FindFirstChildOfClass("Humanoid")
                if hum and hum.Health > 0 and getRoot(obj) then
                    count = count + 1
                    markEnemy(obj)
                    if not seen[obj] then
                        seen[obj] = true
                        notify("CAT HUB | Magnetic Event", "Magnetic enemies have arrived: " .. obj.Name)
                    end
                end
            end
        end

        for enemy, _ in pairs(markers) do
            local hum = enemy and enemy:FindFirstChildOfClass("Humanoid")
            if not enemy or not enemy.Parent or not hum or hum.Health <= 0 then
                seen[enemy] = nil
                unmarkEnemy(enemy)
            end
        end
    ...
    -- Small safe button (detection/marker only; no movement or combat).
    local gui = Instance.new("ScreenGui")
    gui.Name = "CAT_HUB_MagneticUtility"
    gui.ResetOnSpawn = false
    gui.Parent = player:WaitForChild("PlayerGui")

    local button = Instance.new("TextButton")
    button.Name = "MagneticEnemyButton"
    button.Size = UDim2.fromOffset(190, 42)
    button.Position = UDim2.new(0, 12, 0.5, 0)
    button.Text = "🧲 Scan Magnetic Enemies"
    button.Parent = gui

    button.MouseButton1Click:Connect(function()
        local count = scan() or 0
        if count > 0 then
            notify("CAT HUB", tostring(count) .. " Magnetic enemy/enemies detected and marked.")
        else
            notify("CAT HUB", "No Magnetic enemies detected.")
        end
    end)
